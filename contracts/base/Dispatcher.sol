        ...
        if (command < 0x10) {
            ...
            if (command < 0x08) {
                ...
                if (command == Commands.V2_SWAP_EXACT_IN) {
                    (address recipient, uint256 amountIn, uint256 amountOutMin, Route[] memory routes, bool payerIsUser) =
                        abi.decode(inputs, (address, uint256, uint256, Route[], bool));
                    address payer = payerIsUser ? msg.sender : address(this);
                    v2SwapExactInput(recipient.map(), amountIn, amountOutMin, routes, payer);
                } else if (command == Commands.V2_SWAP_EXACT_OUT) {
                    (address recipient, uint256 amountOut, uint256 amountInMax, Route[] memory routes, bool payerIsUser) =
                            abi.decode(inputs, (address, uint256, uint256, Route[], bool));
                    address payer = payerIsUser ? msg.sender : address(this);
                    v2SwapExactOutput(recipient.map(), amountOut, amountInMax, routes, payer);
